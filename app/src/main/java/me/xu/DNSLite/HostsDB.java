...
public class HostsDB extends SQLiteOpenHelper {
    ...
    public Cursor getDnsHostsByGroupAndDomainLike(long gid, String domain) {
        if (gid > 0) {
            return db
                    .rawQuery("SELECT * from dns_hosts where gid=? and domain like ?",
                            new String[]{"" + gid, "%" + domain + "%"});
        } else {
            return db.rawQuery("SELECT * from dns_hosts where domain like ?",
                    new String[]{"%" + domain + "%"});
        }
    }
